...
import firebase_admin, asyncio, os, requests, pyrebase
...
default_app = firebase_admin.initialize_app(cred, {'databaseURL': "https://inducedmesssystem-default-rtdb.asia-southeast1.firebasedatabase.app/"})
    ...
    if request.method == 'POST':
        try:
            ...
            try:
                ...
                proxys = (db.reference(f"/Details/{info['localId']}/Name")).set(name)
                details=auth.get_account_info(newuser["idToken"])
                if details["users"][0]["emailVerified"]== False:
                    return render_template('login.html', s='Verify Your Email')
                resp = make_response(render_template('login.html', s='Login successful'))
